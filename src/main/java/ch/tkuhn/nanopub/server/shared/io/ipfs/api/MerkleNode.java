...
public class MerkleNode {
    ...
    public static MerkleNode fromJSON(Object rawjson) {
        ...
        if (hash == null) {
            Map cid = (Map) json.get("Cid");
            hash = (String)cid.get("/");
        }
        Optional<String> name = json.containsKey("Name") ? Optional.of((String) json.get("Name")): Optional.empty();
        Optional<Integer> size = json.containsKey("Size") ? Optional.<Integer>empty().of(Integer.parseInt(json.get("Size").toString())): Optional.<Integer>empty().empty();
        Optional<Integer> type = json.containsKey("Type") ? Optional.<Integer>empty().of(Integer.parseInt(json.get("Type").toString())): Optional.<Integer>empty().empty();
