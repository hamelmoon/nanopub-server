...
//				return;
	...
	private void collectAndContactPeers() {
		stillAlive();
		isFinished = true;
		List<String> peerUris = new ArrayList<>(NanopubStorageFactory.getInstance().getPeerUris());
	...
	private void checkPeerLists(ServerInfo si) throws Exception {
		...
		for (String peerFromPeer : NanopubServerUtils.loadPeerList(si)) {
			...
			if (myUrl.equals(peerFromPeer)) {
				...
				try {
					NanopubStorageFactory.getInstance().addPeer(peerFromPeer);
				} catch (Exception ex) {
	...
	private void collectNanopubs(ServerInfo si) {
		...
		if (NanopubStorageFactory.getInstance().isFull()) {
		...
		if (!ServerConf.getInfo().getNanopubSurfacePattern().overlapsWith(si.getNanopubSurfacePattern())) {
